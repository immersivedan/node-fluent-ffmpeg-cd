...
module.exports = function(proto) {
  /**
   * Add custom option(s) for the entire ffmpeg process.
   * When passing a single string or an array, each string containing two
   * words is split (eg. mainOptions('-option value') is supported) for
   * compatibility reasons.  This is not the case when passing more than
   * one argument.
   *
   * @example
   * command.mainOptions('option1');
   *
   * @example
   * command.mainOptions('option1', 'option2');
   *
   * @example
   * command.mainOptions(['option1', 'option2']);
   *
   * @method FfmpegCommand#mainOptions
   * @category Custom options
   *
   * @param {...String} options option string(s) or string array
   * @return FfmpegCommand
   */
  proto.mainOptions = function(options) {
    this._mainOptions.clear();
    ...
    this._mainOptions(options.reduce(function(options, option) {
